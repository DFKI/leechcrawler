...
abstract public class URLStreamProvider
{
     ...
     * <br>
     * <b>IMPORTANT</b><br>
     * <li>It is a very good idea that you wrap any stream inside {@link ShiftInitInputStream} also with a {@link TikaInputStream}. Do so with one
     * additional line of code <code>TikaInputStream.get(yourStream)</code></li> <li>Note that a stream will be initialized for sure for determining
     * its mimetype (for reading the magic numbers). Thus the performance win with {@link ShiftInitInputStream} is lost. Mimetype detection with the
     * stream can be prevented by setting the correct Content-Type entry into the metadata object, if you can. Examples: <code>metadata.set("Content-Type",
     ...
     * @throws Exception
     */
    abstract public ShiftInitInputStream getStream(URLName url2getStream, Metadata metadata, ParseContext parseContext) throws Exception;
